...
public class Main extends Application {
    ...
    private void startGame(int timeLimit) {
        ...
        Label controlsLabel = new Label("Controls: [Arrows] Rotate | [Space] Flip | [P] Pause | [R] Restart");
        controlsLabel.setStyle("-fx-text-fill: white; -fx-font-size: 13px; -fx-font-weight: bold;");

        VBox centerArea = new VBox(20);
        centerArea.setAlignment(Pos.CENTER);
        centerArea.getChildren().addAll(boardContainer, controlsLabel);
    ...
    private void handleLevelComplete() {
        SoundManager.playSound("Win.wav");
        currentLevel++; // زيادة رقم الليفل
        boardLogic.loadLevel(currentLevel); // تحميل الخريطة الجديدة

        // إعادة رسم البورد بصرياً (تلوين المربعات المحظورة بالأسود)
        updateBoardUI();

        showComboText("LEVEL " + currentLevel + " START!", WINDOW_WIDTH/2, 100);
        // إيقاف جميع العدادات
        if (timeline != null) timeline.stop();
        if (poolRefiller != null) poolRefiller.stop();
        ...
        savePlayerProgress(); // حفظ العملات

        // تأثير بصري
        explodeBoard();
        showComboText("LEVEL CLEAR! +" + totalReward + " COINS", WINDOW_WIDTH / 2.0, WINDOW_HEIGHT / 2.0);

        // الانتقال للمستوى التالي بعد ثانيتين
        PauseTransition nextLevelDelay = new PauseTransition(Duration.seconds(2));
        nextLevelDelay.setOnFinished(e -> {
            currentLevel++;
            currentLevelTime = Math.max(30, currentLevelTime - 10); // زيادة الصعوبة
            restartGame();
            showComboText("LEVEL " + currentLevel + " START!", WINDOW_WIDTH / 2.0, 100);
        });
        nextLevelDelay.play();
    }
